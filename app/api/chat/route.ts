...
export async function POST(req: Request) {
  ...
  if (characterId === 'mama' && event === 'chibatech') {
    fullSystemPrompt += '\n\n【イベント情報】\nCHIBATECH というテックイベントの会場に出張開店しています。来場者はテクノロジーや起業・プロダクトに関心がある方が多いです。イベントの合間にふらっと立ち寄ってくれた方なので、難しい話はせず、今日の疲れや感情をゆっくり受け止めてあげてね。短い滞在になることも多いので、一言一言を大切に。\n\n【会話の方針】\n来場者が「楽しい」「元気」などポジティブな返答をした場合は、そのまま明るく会話を続けること。無理にモヤモヤや疲れを引き出そうとしない。モヤモヤの話は来場者が自分から話した時だけ受け止める。'
  }
